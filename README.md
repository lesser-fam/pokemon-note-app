...
開発後、自分のランクマッチでもMatchup Noteを使いながら対戦を行いました。

シーズン2、シーズン3ではそれぞれ20戦以上の対戦ログを記録し、どちらのシーズンでもマスターボール級に到達できました。対戦後に「重かった相手」や敗因タグを見返すことで、苦手な相手や負け方の傾向を把握しやすくなりました。

実際に、重かった相手への対策をもとにパーティを改善し、その後の対戦で勝てた試合もありました。アプリを作って終わりではなく、自分で使いながら選出判断と振り返りの流れを確認し、改善につなげられることを重視しています。
...
データベースは `.env.example` と `config/database.php` のデフォルトが `sqlite` になっているため、READMEでは環境構築のしやすさを優先してSQLiteを使用する手順を記載しています。DBサーバーを別途用意せずに動作確認できるため、ポートフォリオ確認時の初期セットアップを簡単にする意図があります。

Laravelの設定上はMySQLにも切り替え可能です。本アプリは、パーティ、対戦ログ、マスターデータ、ルール別データなどを継続的に蓄積するため、長期運用や本番運用ではMySQL等のDBサーバーを利用する構成を想定しています。
