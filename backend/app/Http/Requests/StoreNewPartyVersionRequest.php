...
class StoreNewPartyVersionRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        return [
            ...
            // ID保存用追加分
            'pokemon.*.item_id'         => 'nullable|integer|exists:items,id',
            'pokemon.*.ability_id'      => 'nullable|integer|exists:abilities,id',
            'pokemon.*.nature_id'       => 'nullable|integer|exists:natures,id',
        ...
        $validator->after(function (Validator $validator) {
            ...
            */
            foreach ($pokemonList as $index => $pokemon) {
                $formKey = (string) ($pokemon['form_key'] ?? 'default');

                if ($formKey === 'mega' || str_starts_with($formKey, 'mega-',)) {
                    $validator->errors()->add(
                        "pokemon.{$index}.form_key",
                        '自パーティには通常フォームを登録してください。メガシンカは対戦前選出画面で切り替えられます。',
                    );
                }
            }
